...
class ResolveScriptExporter:
    ...
    def _build_lua(
        ...
        for index, task in enumerate(tasks, start=1):
            clip_id_lines = ", ".join(self._lua_string(item_id) for item_id in task.clip_ids)
            group_id_lines = ", ".join(str(group_id) for group_id in task.source_group_ids)
            shift_value = "nil" if task.shift is None else str(task.shift)
            task_lines.append(
                "\n".join(
                    [
                        "  {",
                        f"    taskId = {self._lua_string(task.task_id)},",
                        f"    layer = {task.layer},",
                        f"    pattern = {self._lua_string(task.pattern_type)},",
                        f"    shift = {shift_value},",
                        f"    sourceGroupIds = {{ {group_id_lines} }},",
                        f"    clipIds = {{ {clip_id_lines} }},",
                        f"    timelineName = {self._lua_string(timeline_names[index - 1])},",
                        f"    outputName = {self._lua_string(output_names[index - 1])},",
                        "  },",
                    ]
                )
            )
        ...
        return "\n".join(
            [
                ...
                "}",
                "",
                "local function resolveClips(clipIds)",
                "  local clips = {}",
                "  for _, clipId in ipairs(clipIds) do",
                "    local numericId = tonumber(clipId)",
                "    local clip = importedClips[numericId]",
                "    if clip == nil then",
                "      error('Imported clip is unavailable for clip id: ' .. tostring(clipId))",
                "    end",
                "    clips[#clips + 1] = clip",
                "  end",
                "  return clips",
                "end",
                "",
                "local function queueTask(task)",
                "  local timelineName = task.timelineName",
                ...
                "  local clips = resolveClips(task.clipIds)",
